...
# This script takes a bam file or a list of bam files (filename must end 
# ".list") and runs variant calling using the HaplotypeCaller in gVCF mode
#
#    InpFil - (required) - Path to Bam file to be aligned. Alternatively a file 
#             with a list of bams can be provided and the script run as an array
#             job. List file name must end ".list"
#
#    RefFil - (required) - shell file containing variables with locations of 
#             reference files, jar files, and resource directories; see list 
#             below for required variables
#
#    LogFil - (optional) - File for logging progress
#
#    Flag - C - FullCadd - Annotate with full CADD database. The default is to 
#             use the caddgt10 database, which contains only variants with CADD
#             scores that are within top 10% percentile. Using the full CADD 
#             database significantly increases the amount of time required for 
#             annotation, especially for larger vcfs (can mean the difference 
#             between 30 mins and several hours)
#
#    Flag - P - PipeLine - call the next step in the pipeline at the end of the
#             job
#
...
# N.B. : The perl script "table_annovar_cadd.pl", which is used below, is a 
#        modified version of the table_annovar.pl script that was released
#        independent of the main bundle on 24th February 2014 (see annovar 
#        homepage).  The "_cadd" version has added lines to allow for the 
#        inclusion of the phred-scaled cadd score from the cadd or caddgt10 
#        annovar databases. In the normal perl script only the raw cadd scores
#        are added to the annotation.

## This file also requires exome.lib.sh - which contains various functions used
#  throughout the Exome analysis scripts; this file should be in the same 
#  directory as this script

###############################################################

#set default arguments
usage="ExmVC.3.b38.AnnotatewithANNOVAR.sh -i <InputFile> -r <reference_file> 
                                   -l <logfile> -PH

     -i (required) - Path to VCF file or \".list\" file containing a multiple 
         paths
         
     -r (required) - shell file containing variables with locations of 
         reference files and resource directories
     
     -l (optional) - Log file
     
     -C (flag) - Annotate with full CADD database
     
     -P (flag) - Call next step of exome analysis pipeline after completion of 
         script
...
# Handle the command line imput
while getopts i:r:l:CXFPBH opt; 
  do
    case "$opt" in
      i) InpFil="$OPTARG";;
      r) RefFil="$OPTARG";; 
...
#Call the Reference File to load variables
RefFil = `readlink -f $RefFil`
source $RefFil

#Load script library
source $EXOMPPLN/exome.lib.sh 
#library functions begin "func" #library functions begin "func"

#Set local Variables
##Set local parameters
ArrNum = $SGE_TASK_ID
funcFilfromList # if the input is a list get the appropriate input file for this
                # job of the array --> $InpFil
VcfFil = $InpFil  #input vcf file
VcfNam = `basename $VcfFil | sed s/.gz$// | sed s/.vcf$// | \
         sed s/.rawvariants$//`  #basename for outputs
         
if [[ -z $LogFil ]]; 
  then 
  	LogFil=$VcfNam.AnnVCF.log; 
fi  # a name for the log file
...
GatkLog =$ VcfNam.gatklog  # a log for GATK to output to, this is then trimmed 
	                       # and added to the script log
infofields = "-A AlleleBalance -A BaseQualityRankSumTest -A Coverage \
              -A HaplotypeScore -A HomopolymerRun -A MappingQualityRankSumTest\
              -A MappingQualityZero -A QualByDepth -A RMSMappingQuality \
              -A SpanningDeletions -A FisherStrand -A InbreedingCoeff" 
              # Annotation fields for GATK to output into vcf files
...
## Convert VCF to ANNOVAR input file using ANNOVAR - use a trimmed vcf to all 
## possible alternate alleles using the withfreq flag (using all samples slows 
## this down considerably as annovar calculated the allele frequencies)
...
StepCmd = "vcftools --vcf $VcfFil --indv $OneSam --recode --out TEMP.$VcfFil;
 convert2annovar.pl -includeinfo -allsample -withfreq -format vcf4 \
                    TEMP.$VcfFil.recode.vcf -outfile $TmpVar;
 cut -f 1-5,9-13 $TmpVar > $TmpVar.2;
 mv $TmpVar.2 $TmpVar"
 
if [[ $FilTyp == "gz" ]]; 
  then 
  	StepCmd=`echo $StepCmd | sed s/--vcf/--gzvcf/g`; 
fi
funcRunStep
...
################################
# change caddgt10 to cadd13gt10
################################

#StepCmd="table_annovar.pl $TmpVar $ANNHDB --buildver hg38 --remove -protocol refGene,esp6500siv2_all,esp6500siv2_aa,esp6500siv2_ea,1000g2014oct_all,1000g2014oct_eur,1000g2014oct_amr,1000g2014oct_eas,1000g2014oct_afr,1000g2014oct_sas,exac03,ljb26_all,cadd13gt10,caddindel,cosmic70,genomicSuperDups -operation g,f,f,f,f,f,f,f,f,f,f,f,f,f,f,r -otherinfo  -nastring %%%  --outfile $AnnFil"
##Leon StepCmd without cadd
StepCmd = "table_annovar.pl $TmpVar $ANNHDB --buildver hg38 --remove \
           -protocol refGene,esp6500siv2_all,esp6500siv2_aa,esp6500siv2_ea,\
           1000g2015aug_all,1000g2015aug_eur,1000g2015aug_amr,1000g2015aug_eas,\
           1000g2015aug_afr,1000g2015aug_sas,exac03,ljb26_all,cadd1.4all,\
           cadd1.4indel,cosmic70,genomicSuperDups -operation g,f,f,f,f,f,f,f,f,\
           f,f,f,f,f,f,r -otherinfo  -nastring %%%  --outfile $AnnFil"
echo $StepCmd
...
funcRunStep
AnnFil = $AnnFil.hg38_multianno.txt
rm -f $TmpVar
